...
from Pinyin2Hanzi.implement import DefaultHmmParams
from Pinyin2Hanzi.viterbi import viterbi

hmmparams = DefaultHmmParams()

result = viterbi(hmm_params=hmmparams, observations=('ni', 'hao', 'a'), path_num = 5, log = False)
for item in result:
    print item.score, '/'.join(item.path)

print 20*'--'

result = viterbi(hmm_params=hmmparams, observations=('ni', 'hao', 'a'), path_num = 2, log = False)
for item in result:
    print item.score, '/'.join(item.path)
